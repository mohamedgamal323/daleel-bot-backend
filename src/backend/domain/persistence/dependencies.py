...
def get_user_repository() -> UserRepository:
    ...
    if getattr(settings, 'USE_MONGODB', False):
        from src.backend.infrastructure_persistence.mongo_user_repo import MongoUserRepository
...
def get_domain_repository() -> DomainRepository:
    ...
    settings = get_settings()
    if getattr(settings, 'USE_MONGODB', False):
        from src.backend.infrastructure_persistence.mongo_domain_repo import MongoDomainRepository
        return MongoDomainRepository()
    else:
        if _domain_repo_instance is None:
            from src.backend.infrastructure_persistence.memory_domain_repo import MemoryDomainRepository
...
def get_category_repository() -> CategoryRepository:
    ...
    settings = get_settings()
    if getattr(settings, 'USE_MONGODB', False):
        from src.backend.infrastructure_persistence.mongo_category_repo import MongoCategoryRepository
        return MongoCategoryRepository()
    else:
        if _category_repo_instance is None:
            from src.backend.infrastructure_persistence.memory_category_repo import MemoryCategoryRepository
...
def get_asset_repository() -> AssetRepository:
    ...
    settings = get_settings()
    if getattr(settings, 'USE_MONGODB', False):
        from src.backend.infrastructure_persistence.mongo_asset_repo import MongoAssetRepository
